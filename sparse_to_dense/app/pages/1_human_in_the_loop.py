...
def copy_and_organize_images(directory, interactions):
    ...
    for i, interaction in enumerate(interactions):
        if interaction.get('validation', False):  # Only copy validated images
            image_path = os.path.join(directory, interaction['image_path'])
            label = interaction['user_label']
            target_folder = f"feature-store/{label}"
...
def app():
    ...
    if selected_directory:
        full_path = os.path.join(base_path, selected_directory)
        json_file = get_files(full_path)
        ...
        if json_file:
            ...
            if all_validated:
                if st.button("Push to feature-store"):
                    feature_store_pushed = copy_and_organize_images(full_path, interactions)
