...
def video2imgs(path, fps):  #test-ws/sparse-label/clip1.mp4 -> clip_frames/clip1
    # id = path.split("/")[-1][:-4]  # clip1
    id = os.path.basename(path) #clip1.mp4
    id = id.split('.')[0] #clip1
    save_dir = os.path.join("clip_frames", id) #clip_frames/clip1
...
def DenseModule(frame_dir, image_names, label, sampling, size): ##clip_frames/clip1
        ...
        if (
            ...
            while True:
                ...
                if key == ord("0"):
                    ...
                    new_dir = os.path.join('0', new_dir)
                    cv2.imwrite(new_dir, image)
                    break

                if key == ord("1"):
                    new_dir = os.path.basename(frame_dir) + "_" + image_name
                    new_dir = os.path.join('1', new_dir)
                    cv2.imwrite(new_dir, image)
                    break
...
def main():
    ...
    for data in datas:
        ...
        path = os.path.join(folder, data["clip_name"])  # test-ws/sparse-label/clip1.mp4
